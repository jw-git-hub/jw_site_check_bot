...
def test_queued_with_one_site_ahead_uses_singular_verb():
    assert "There is 1 site ahead of yours — about 2 min." in rich_text(replies.queued(TEXTS, "en", 1, 2))
    assert "Передо мной ещё 1 сайт — ждать примерно 2 мин." in rich_text(replies.queued(TEXTS, "ru", 1, 2))
...
def test_unknown_failure_code_falls_back_to_measure_failed():
    assert "Не получилось измерить" in rich_text(replies.failure(TEXTS, "ru", "что-то новое"))


def test_limit_user_counts_checks_and_hours():
    assert ("Лимит — 10 проверок в сутки. Следующая будет доступна через 3 ч."
            in rich_text(replies.limit_user(TEXTS, "ru", 10, 3)))
    assert "The limit is 10 checks a day." in rich_text(replies.limit_user(TEXTS, "en", 10, 3))
...
def test_busy_names_the_site():
    assert "Сначала закончу с example.com" in rich_text(replies.busy(TEXTS, "ru", "example.com"))
